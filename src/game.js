...
/* ---------- 第三新东京市环境 ---------- */
let cityDown = false;                       // Boss 波：要塞都市转换（大楼收纳）
const cityBuildings = [];
(function buildTokyo3() {
  ...
  function bld(x, z, w, d, h) {
    ...
    const b = new THREE.Mesh(geo('box', w, h, d), bmats[(rnd() * 4) | 0]);
    b.position.y = h / 2; b.castShadow = b.receiveShadow = true;
    grp.add(b);
    const bands = 1 + ((h / 26) | 0);
    for (let i = 0; i < bands; i++) {                                       // 发光窗带
      const st = new THREE.Mesh(geo('box', w + 0.4, 0.9, d + 0.4), wmat);
    ...
    scene.add(grp);
    cityBuildings.push({ grp, h });
  }
  for (let i = 0; i < 13; i++)                                              // 北侧天际线（高层）
    bld(-390 + i * 64 + rnd() * 22, -286 - rnd() * 120, 20 + rnd() * 16, 18 + rnd() * 14, 42 + rnd() * 62);
  for (let i = 0; i < 6; i++)                                               // 西侧楼群
    bld(-478 - rnd() * 90, -170 + i * 62, 20 + rnd() * 14, 20 + rnd() * 12, 34 + rnd() * 44);
  for (let i = 0; i < 7; i++)                                               // 南侧近景（低层，避免挡镜头）
    bld(-330 + i * 110 + rnd() * 30, 292 + rnd() * 60, 18 + rnd() * 14, 16 + rnd() * 12, 20 + rnd() * 22);
  for (let i = 0; i < 4; i++)                                               // 东侧（NERV 外围）
...
function makePad() {
  ...
  const plate = new THREE.Mesh(geo('box', 33, 1.5, 33), mat(0x232b38, { roughness: 0.85 }));
  plate.position.y = 0.75; plate.receiveShadow = true; pad.add(plate);
  const inner = new THREE.Mesh(geo('box', 22, 0.7, 22), mat(0x1a212c));
  inner.position.y = 1.6; pad.add(inner);
  for (const [px, pz] of [[1, 1], [1, -1], [-1, 1], [-1, -1]]) {            // 黄色警示角
    const c1 = new THREE.Mesh(geo('box', 8, 0.6, 2.4), mat(0xd8b64a));
    c1.position.set(px * 12.5, 1.85, pz * 13.5); pad.add(c1);
    const c2 = new THREE.Mesh(geo('box', 2.4, 0.6, 8), mat(0xd8b64a));
    c2.position.set(px * 13.5, 1.85, pz * 12.5); pad.add(c2);
  }
  return pad;
}
...
const baseGroup = (() => {
  ...
  const hex = P(g, 'cyl', [30, 33, 2.6, 6], mat(0x1d1730, { roughness: 0.6 }), 0, 1.3, 0);
  hex.receiveShadow = true;
  P(g, 'tor', [30, 0.5, 6, 6], emat(0x9a6bff, 1.2), 0, 2.8, 0, Math.PI/2, 0, 0).castShadow = false;
  const cone = P(g, 'cone', [21, 42, 4], mat(0x171029, { roughness: 0.35, metalness: 0.3 }), 0, 23, 0, 0, Math.PI/4, 0);
  const edges = new THREE.LineSegments(
    new THREE.EdgesGeometry(cone.geometry),
    new THREE.LineBasicMaterial({ color: 0x9a6bff, transparent: true, opacity: 0.8 }));
  edges.position.y = 23; edges.rotation.y = Math.PI/4;
  g.add(edges);
...
function resetGame() {
  if (G) {
    for (const t of G.towers) { scene.remove(t.mesh); if (t.pad) scene.remove(t.pad); }
...
function announce(txt, color, size) {
  const stack = G.texts.filter(t => t.screen).length;   // 多条公告自动错行
  G.texts.push({ x: 0.5, y: 0.32 + stack * 0.055, txt, color, size: size || 20, ttl: 1.4, life: 1.4, screen: true });
}
...
function placeTower(key, c, r) {
  ...
  scene.add(t.mesh);
  t.pad = makePad();
  t.pad.position.copy(gw(t.x, t.y, 0));
  scene.add(t.pad);
  t.riseT = 0.5;                             // 弹射升起动画
  G.towers.push(t);
...
function startWave() {
  ...
  if (waveHasBoss(G.wave)) {
    cityDown = true;
    announce('要塞都市转换 · 装甲大楼收纳', '#ff9a3d', 17);
    SFX.alarm();
...
function syncMeshes(dt) {
  for (const t of G.towers) {
    if (t.riseT > 0) {                       // 从弹射井升起
      ...
      const k = t.riseT / 0.5;
      t.mesh.position.y = -36 * k * k;
      if (t.riseT === 0) addParts(t.x, t.y, 2, '#9fb4cc', 8, 55);
    }
  ...
  for (const b of cityBuildings) {          // 要塞都市转换
    const ty = cityDown ? -(b.h + 3) : 0;
    b.grp.position.y += (ty - b.grp.position.y) * Math.min(1, dt * 2.0);
  }
...
function drawOverlay() {
  ...
  if (bp) { octx.fillStyle = '#b18aff'; octx.fillText('NERV 本部', bp.x, bp.y); }
  // 不可部署提示
